...
function mapGenerator(name, la, lo) {
  ...
  if (isMobile()) {
    var contentString = ["<div>", "   <h5>" + name + "</h5>", "</div>"].join(
      ""
    );
  } else {
    var contentString = [
      "<div style = 'border-radius: 50px; padding:10px;'>",
      "   <h5>" + name + "</h5><br>",
      '   <a target="_blank" href="http://map.naver.com/search/청주 ' +
        name +
        '" style="color: black;" >위치 자세히 보기</a>',
      "</div>",
    ].join("");
  }
